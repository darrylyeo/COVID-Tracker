...
def query_from_config(db, config, query_index):
        ...
        if config["aggregation"] == "fiftyStates" or config["aggregation"] == "usa":
                group_dict = {
                        ...
                        ("s" + config["analysis"][query_index]["task"]["track"] if
                        config["analysis"][query_index]["task"].keys()[0] == "track" else
                ...
                group_dict = { 
                        "$group": {
		                "_id": (config["aggregation"] if 
                                        (config["aggregation"] == "state" and config["target"] is list) else 
                                        "$date" if
                                ...
                                "array": {"$push": 
                                        (config["analysis"][query_index]["task"]["track"] if
                                        config["analysis"][query_index]["task"].keys()[0] == "track" else
                                        config["analysis"][query_index]["task"].keys()[0])},
                                "dateArray": {"$push": "$date"},
